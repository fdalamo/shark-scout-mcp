...
const OUTCOME_PATH=process.env.CANONICAL_RESCUE_OUTCOME_PATH||"/data/canonical-rescue/outcomes.json";
const PARSER_VERSION=process.env.CANONICAL_RESCUE_PARSER_VERSION||"raw_meta_v2_decision_aware";
const TRANSIENT_RETRY_MS=Math.max(60_000,Number(process.env.CANONICAL_RESCUE_TRANSIENT_RETRY_MS||86_400_000));
...
const TERMINAL_REASONS=new Set(["WALLET_NOT_RESOLVED_IN_RAW_META","CPI_TOKEN_OWNER_UNRESOLVED","TOKEN_OWNER_UNRESOLVED","CPI_QUOTE_ONLY_OR_WSOL_TEMP_ACCOUNT","QUOTE_ONLY_OR_WSOL_TEMP_ACCOUNT","CPI_NO_MATERIAL_WALLET_DELTA","RAW_META_NO_MATERIAL_WALLET_DELTA"]);

type Budget={remaining:number;attempted:number;cacheHits:number;resolved:number;failed:number;suppressed:number;providers:Record<string,number>;reasons:Record<string,number>};
type Outcome={parserVersion:string;wallet:string;signature:string;updatedAt:string;resolved:boolean;reason:string;terminal:boolean;attempts:number};
type OutcomeLedger={schemaVersion:1;parserVersion:string;updatedAt:string;outcomes:Record<string,Outcome>};
export type RescueStats={wallet:string;candidates:number;attempted:number;cacheHits:number;fetched:number;resolved:number;failed:number;suppressed:number;reasonCodes:Record<string,number>};
...
function outcomeKey(wallet:string,signature:string){return `${wallet}:${signature}`;}
function now(){return new Date().toISOString();}
async function readOutcomeLedger():Promise<OutcomeLedger>{const x=await readJson(OUTCOME_PATH);return x?.schemaVersion===1&&x?.outcomes?{schemaVersion:1,parserVersion:String(x.parserVersion||PARSER_VERSION),updatedAt:String(x.updatedAt||now()),outcomes:x.outcomes}: {schemaVersion:1,parserVersion:PARSER_VERSION,updatedAt:now(),outcomes:{}};}
function retrySuppressed(o:Outcome|undefined){if(!o||o.parserVersion!==PARSER_VERSION||o.resolved)return false;if(o.terminal)return true;const age=Date.now()-Date.parse(o.updatedAt);return Number.isFinite(age)&&age<TRANSIENT_RETRY_MS;}
...
export function createRescueBudget(maxCalls?:number):Budget{const requested=maxCalls??Number(process.env.CANONICAL_RESCUE_MAX_CALLS||48);const cap=Math.max(0,Math.min(64,Number(requested)));return{remaining:cap,attempted:0,cacheHits:0,resolved:0,failed:0,suppressed:0,providers:{},reasons:{}};}

export async function rescueRows(rows:any[],wallet:string,budget:Budget):Promise<{rows:any[];stats:RescueStats}> {const out=[...rows],stats:RescueStats={wallet,candidates:0,attempted:0,cacheHits:0,fetched:0,resolved:0,failed:0,suppressed:0,reasonCodes:{}};let walletCalls=0,ledger=await readOutcomeLedger(),ledgerDirty=false;
  for(let i=0;i<out.length;i++){const tx=out[i],before=classifyEnhancedTransaction(tx,wallet);if(before.trade||!before.reason||!ESCALATABLE.has(before.reason))continue;stats.candidates++;if(!tx?.signature){inc(stats.reasonCodes,"MISSING_SIGNATURE");continue;}const sig=String(tx.signature),okey=outcomeKey(wallet,sig),priorOutcome=ledger.outcomes[okey];if(retrySuppressed(priorOutcome)){stats.suppressed++;budget.suppressed++;inc(stats.reasonCodes,`SUPPRESSED_${priorOutcome?.reason||"UNRESOLVED"}`);inc(budget.reasons,`SUPPRESSED_${priorOutcome?.reason||"UNRESOLVED"}`);continue;}let raw=await readJson(cachePath(sig));if(raw){stats.cacheHits++;budget.cacheHits++;}else{if(walletCalls>=MAX_PER_WALLET||budget.remaining<=0||!providerFabricEnabled()){inc(stats.reasonCodes,budget.remaining<=0?"RESCUE_BUDGET_EXHAUSTED":!providerFabricEnabled()?"PROVIDER_FABRIC_DISABLED":"WALLET_RESCUE_CAP");continue;}walletCalls++;budget.remaining--;budget.attempted++;stats.attempted++;try{const got=await routedRpc("getTransaction",[sig,{encoding:"jsonParsed",commitment:"confirmed",maxSupportedTransactionVersion:0}]);raw=got.result;inc(budget.providers,got.provider);stats.fetched++;if(raw)await atomic(cachePath(sig),raw);else{inc(stats.reasonCodes,"RPC_TRANSACTION_NULL");ledger.outcomes[okey]={parserVersion:PARSER_VERSION,wallet,signature:sig,updatedAt:now(),resolved:false,reason:"RPC_TRANSACTION_NULL",terminal:false,attempts:Number(priorOutcome?.attempts||0)+1};ledgerDirty=true;}}catch(e){stats.failed++;budget.failed++;inc(stats.reasonCodes,"RPC_ESCALATION_FAILED");ledger.outcomes[okey]={parserVersion:PARSER_VERSION,wallet,signature:sig,updatedAt:now(),resolved:false,reason:"RPC_ESCALATION_FAILED",terminal:false,attempts:Number(priorOutcome?.attempts||0)+1};ledgerDirty=true;continue;}}
    if(!raw)continue;const augmented=augmentWithRawMeta(tx,raw,wallet),after=classifyEnhancedTransaction(augmented,wallet);out[i]=augmented;if(after.trade){stats.resolved++;budget.resolved++;inc(stats.reasonCodes,"RESOLVED_RAW_META");ledger.outcomes[okey]={parserVersion:PARSER_VERSION,wallet,signature:sig,updatedAt:now(),resolved:true,reason:"RESOLVED_RAW_META",terminal:false,attempts:Number(priorOutcome?.attempts||0)+1};ledgerDirty=true;}else{const reason=augmented?._canonicalRescueReason||after.reason||"UNRESOLVED_AFTER_RAW_META",terminal=TERMINAL_REASONS.has(reason);inc(stats.reasonCodes,reason);inc(budget.reasons,reason);ledger.outcomes[okey]={parserVersion:PARSER_VERSION,wallet,signature:sig,updatedAt:now(),resolved:false,reason,terminal,attempts:Number(priorOutcome?.attempts||0)+1};ledgerDirty=true;}
  }
  if(ledgerDirty){ledger.updatedAt=now();ledger.parserVersion=PARSER_VERSION;await atomic(OUTCOME_PATH,ledger);}return{rows:out,stats};
}
