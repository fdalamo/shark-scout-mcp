...
export function createRescueBudget(maxCalls?:number):Budget{const requested=maxCalls??Number(process.env.CANONICAL_RESCUE_MAX_CALLS||48);const cap=Math.max(0,Math.min(64,Number(requested)));return{remaining:cap,attempted:0,cacheHits:0,resolved:0,failed:0,providers:{},reasons:{}};}
