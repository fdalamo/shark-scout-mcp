...
type StageResult={name:string;phase:Phase;workClass:WorkClass;status:"SUCCESS"|"FAILED"|"TIMED_OUT"|"SKIPPED_BUDGET"|"SKIPPED_EVENT";durationMs:number;exitCode:number|null;signal:NodeJS.Signals|null};
...
// HOT is live/prospective truth. COLD is expendable discovery/backfill. SYSTEM preserves integrity/reporting.
const stages:Stage[]=[
...
function skipped(stage:Stage,status:"SKIPPED_BUDGET"|"SKIPPED_EVENT"="SKIPPED_BUDGET"):StageResult{return{name:stage.name,phase:stage.phase,workClass:stage.workClass,status,durationMs:0,exitCode:null,signal:null};}
...
async function main(){const pipelineStarted=Date.now(),startedAt=new Date(pipelineStarted).toISOString(),deadline=pipelineStarted+PIPELINE_BUDGET_MS,results:StageResult[]=[];let lastPhase:Phase|null=null,canonicalSnapshotTaken=false;const queueCounts={HOT:stages.filter(x=>x.workClass==="HOT").length,COLD:stages.filter(x=>x.workClass==="COLD").length,SYSTEM:stages.filter(x=>x.workClass==="SYSTEM").length};log("shark_scout_pipeline_started",{version:6,stageCount:stages.length,queueCounts,budgetMs:PIPELINE_BUDGET_MS,finalizeReserveMs:FINALIZE_RESERVE_MS,eventDrivenHotRefresh:true});
...
for(const stage of stages){if(stage.phase!==lastPhase){lastPhase=stage.phase;log("shark_scout_phase_started",{phase:lastPhase,elapsedMs:Date.now()-pipelineStarted,remainingMs:deadline-Date.now()});}const remaining=Math.max(0,deadline-Date.now());
  ...
  if(stage.workClass==="COLD"&&remaining<=FINALIZE_RESERVE_MS){const r=skipped(stage);results.push(r);log("shark_scout_stage_finished",r);continue;}
  if(stage.name==="canonical_snapshot_pre"&&remaining<=FINALIZE_RESERVE_MS+6*MINUTE){const r=skipped(stage);results.push(r);log("shark_scout_stage_finished",r);continue;}
  if(protectedCanonicalStages.has(stage.name)&&!canonicalSnapshotTaken){const r=skipped(stage);results.push(r);log("shark_scout_stage_finished",r);continue;}
  // MONITOR always polls once per hourly run. FINALIZE only repeats Paper/Dip if source-wallet signatures changed during the run (or forced/error fail-open).
  if(eventRefreshStages.has(stage.name)&&!hotRefreshNeeded()){const r=skipped(stage,"SKIPPED_EVENT");results.push(r);log("shark_scout_stage_finished",r);continue;}
  const reserved=stage.workClass==="HOT"||stage.workClass==="SYSTEM"?0:FINALIZE_RESERVE_MS;const allowed=Math.max(1000,Math.min(stage.timeoutMs,deadline-Date.now()-reserved));const result=await runStage(stage,allowed);results.push(result);log("shark_scout_stage_finished",result);if(stage.name==="canonical_snapshot_pre"&&result.status==="SUCCESS")canonicalSnapshotTaken=true;if(stage.name==="canonical_restore"&&result.status==="SUCCESS")canonicalSnapshotTaken=false;if(result.status!=="SUCCESS"&&result.status!=="SKIPPED_EVENT"&&!stage.continueOnFailure){const restored=await emergencyRestore(`critical_stage_${stage.name}_${result.status}`);log("shark_scout_pipeline_aborted",{stage:stage.name,result,startedAt,canonicalSnapshotTaken,emergencyRestoreOk:restored});process.exitCode=1;return;}}
if(canonicalSnapshotTaken){const restored=await emergencyRestore("pipeline_end_snapshot_open");if(!restored)process.exitCode=1;}const failed=results.filter(x=>x.status==="FAILED"||x.status==="TIMED_OUT"),skippedRows=results.filter(x=>x.status==="SKIPPED_BUDGET"),eventSkipped=results.filter(x=>x.status==="SKIPPED_EVENT");log("shark_scout_pipeline_complete",{startedAt,finishedAt:new Date().toISOString(),durationMs:Date.now()-pipelineStarted,stageCount:results.length,failedCount:failed.length,skippedBudgetCount:skippedRows.length,skippedEventCount:eventSkipped.length,failedStages:failed,skippedStages:skippedRows.map(x=>({name:x.name,workClass:x.workClass})),eventSkippedStages:eventSkipped.map(x=>x.name)});if(failed.length&&process.exitCode!==1)process.exitCode=2;}
