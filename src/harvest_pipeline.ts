...
async function main(){const pipelineStarted=Date.now(),startedAt=new Date(pipelineStarted).toISOString(),deadline=pipelineStarted+PIPELINE_BUDGET_MS,results:StageResult[]=[];let lastPhase:Phase|null=null,canonicalSnapshotTaken=false;const queueCounts={HOT:stages.filter(x=>x.workClass==="HOT").length,COLD:stages.filter(x=>x.workClass==="COLD").length,SYSTEM:stages.filter(x=>x.workClass==="SYSTEM").length};log("shark_scout_pipeline_started",{version:10,stageCount:stages.length,queueCounts,budgetMs:PIPELINE_BUDGET_MS,finalizeReserveMs:FINALIZE_RESERVE_MS,eventDrivenHotRefresh:true,sharedHeliusHardQuotaCircuit:true,odinResearchCore:"v0.43.1"});
...
for(const stage of stages){if(stage.phase!==lastPhase){lastPhase=stage.phase;log("shark_scout_phase_started",{phase:lastPhase,elapsedMs:Date.now()-pipelineStarted,remainingMs:deadline-Date.now()});}const remaining=Math.max(0,deadline-Date.now());
  ...
  // A hard specialty-Helius quota is provider-wide. Reuse the prior Gauntlet's persisted circuit evidence so direct callers do not each burn a doomed request.
  // Gauntlet itself remains allowed as the hourly recovery probe; when it succeeds, the next run automatically closes this circuit.
  if(heliusSpecialtyDirectStages.has(stage.name)){const circuit=heliusHardQuotaCoolingDown();if(circuit){const r=skipped(stage,"SKIPPED_PROVIDER");results.push(r);log("shark_scout_helius_specialty_stage_skipped",{stage:stage.name,reason:"RECENT_HARD_QUOTA",...circuit});log("shark_scout_stage_finished",r);continue;}}
