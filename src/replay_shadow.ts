...
async function main(){
  ...
  const out={schemaVersion:2,event:"shark_scout_replay_shadow_complete",startedAt,finishedAt:new Date().toISOString(),walletsExamined:rows.length,zeroReplayRecoveredTrusted:recovered.length,moreTrustedReplayFound:more.length,lowerTrustedReplayDisagreements:lower.length,suspiciousEconomicsCount:suspicious.length,topRecovered:recovered.sort((a,b)=>Number(b.shadowReplay.trusted.netSol||-999)-Number(a.shadowReplay.trusted.netSol||-999)).slice(0,10),largestDisagreements:[...rows].sort((a,b)=>Math.abs(b.tradeDelta)-Math.abs(a.tradeDelta)).slice(0,10),suspiciousEconomics:suspicious.slice(0,10),rows,notes:["Shadow-only diagnostic: never changes gauntlet verdicts or promotion eligibility.","v2 economics require compatible quote assets on both legs; token-to-token and quote-mismatch trades never fabricate SOL PnL.","Native SOL deltas are fee-neutralized only when the wallet is proven fee payer; WSOL and native SOL are canonicalized as one quote.","Extreme uncorroborated source ROI is retained in raw audit output but quarantined from trusted replay.","Every non-trusted/outlier replay exposes transaction signatures and quote provenance for manual or provider cross-validation."]};
