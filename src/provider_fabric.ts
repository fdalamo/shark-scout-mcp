...
const MIN_INTERVAL:Record<ProviderName,number>={
  ...
  // Verified free ceiling is 10 RPS; default ~4 RPS intentionally leaves headroom.
...
export function methodAwareOrder(method:string):ProviderName[]{
  ...
  return uniqueOrder(["chainstack","solana_tracker","alchemy","helius"],BASE_ORDER);
}
...
export async function routedRpc(method:string,params:unknown[]=[],opts:{preferred?:ProviderName[];verify?:boolean}={}){if(!ENABLED)throw new Error("provider_fabric_disabled");const state=await readState();const candidates=(opts.preferred?.length?opts.preferred:methodAwareOrder(method)).filter(p=>Boolean(URLS[p]));if(!candidates.length)throw new Error("provider_fabric_no_rpc_provider");let last:unknown;for(const p of candidates){if(isCooling(p))continue;const h=health(state,p);recordAttempt(h,p,method);try{const x=await callProvider(p,method,params);recordSuccess(h,method,x.latencyMs);cooldownUntil[p]=0;await saveState(state);if(opts.verify&&p!=="helius"&&URLS.helius){const vh=health(state,"helius");recordAttempt(vh,"helius",method);try{const v=await callProvider("helius",method,params);recordSuccess(vh,method,v.latencyMs);if(JSON.stringify(v.result)!==JSON.stringify(x.result))throw new Error(`provider_disagreement:${p}:helius`);await saveState(state);}catch(e:any){recordFailure(vh,method,e?.status===429);vh.lastError=String(e).slice(0,500);await saveState(state);throw e;}}return{provider:p,result:x.result};}catch(e:any){last=e;recordFailure(h,method,e?.status===429);applyFailureCooldown(p,e);h.lastError=String(e).slice(0,500);await saveState(state);}}
const cooling=candidates.map(p=>({p,until:cooldownUntil[p]||0})).filter(x=>x.until>Date.now()).sort((a,b)=>a.until-b.until);if(cooling.length===candidates.length&&cooling.length){await sleep(Math.max(0,cooling[0].until-Date.now()));return routedRpc(method,params,opts);}throw last instanceof Error?last:new Error(String(last||"provider_fabric_rpc_failed"));}
export async function providerFabricReport(){const state=await readState(),now=Date.now();return{event:"shark_scout_provider_fabric",enabled:ENABLED,configured:configuredRpcProviders(),baseOrder:BASE_ORDER,unitModels:{chainstack:"logical request unit; exact RU depends on method",solana_tracker:"estimated documented credits by method",alchemy:"logical request unit; exact CU not inferred",helius:"logical request unit; exact credits not inferred"},providers:Object.fromEntries(Object.entries(state.providers).map(([k,v])=>[k,{...v,successRate:v.requests?v.successes/v.requests:null,avgLatencyMs:v.successes?v.latencyMsTotal/v.successes:null,cooldownRemainingMs:Math.max(0,(cooldownUntil[k as ProviderName]||0)-now)}]))};}
