...
function followerLots(cache:any){
  ...
  for(const e of events){
    ...
    while(remain>1e-12&&q.length){
      ...
      if(lot.qtyRemaining<=lot.qtyInitial*(1-ACTUAL_CLOSE_FRACTION)){lot.resolvedAt=new Date(Number(e.ts)*1000).toISOString();q.shift();}
