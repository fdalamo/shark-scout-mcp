...
async function rpc(method:string,params:any[]){const c=new AbortController(),t=setTimeout(()=>c.abort(),TIMEOUT);try{const r=await fetch(`https://mainnet.helius-rpc.com/?api-key=${encodeURIComponent(KEY!)}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({jsonrpc:"2.0",id:1,method,params}),signal:c.signal}),raw=await r.text();let b:AnyObj|null=null;if(raw){try{b=JSON.parse(raw) as AnyObj;}catch{const snippet=raw.replace(/\s+/g," ").trim().slice(0,160);if(!r.ok)throw new Error(`rpc_${method}:http_${r.status}${snippet?`:${snippet}`:""}`);throw new Error(`rpc_${method}:invalid_json_response${snippet?`:${snippet}`:""}`);}}if(!r.ok||b?.error){const detail=String(b?.error?.message||b?.error?.code||`http_${r.status}`);throw new Error(`rpc_${method}:${detail}`);}return b?.result;}finally{clearTimeout(t);}}
...
async function history(current:Map<string,number>,accounts:TokenAccount[]){
  ...
  for(const s of sigRows){const sig=String(s?.signature||"");if(!sig)continue;if(cache.seen[sig]){cacheHits++;continue;}try{const tx=await rpc("getTransaction",[sig,{encoding:"jsonParsed",commitment:"confirmed",maxSupportedTransactionVersion:1}]);rpcFetched++;if(tx?.meta&&!tx.meta.err){const a=analyze(tx,sig,s?.blockTime??tx?.blockTime??null);if(a.event)cache.events[sig]=a.event;if(a.movement)cache.movements[sig]=a.movement;if(!a.event&&!a.movement)ambiguous++;}cache.seen[sig]=1;}catch{ambiguous++;}}
...
async function main(){
  const startedAt=now(),base:any={event:"shark_scout_portfolio_complete",version:4,startedAt,enabled:Boolean(KEY&&valid(WALLET)),walletConfigured:Boolean(WALLET),errors:[],warnings:[]};if(!KEY||!valid(WALLET)){console.log(JSON.stringify({...base,finishedAt:now(),skipped:!KEY?"missing_helius_key":"missing_or_invalid_wallet"}));return;}
