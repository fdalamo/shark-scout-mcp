...
app.get("/health",(_req,res)=>{const c=reliability.status();res.json({ok:true,service:"shark-scout-alchemy-ab",version:VERSION,signingKeyConfigured:Boolean(SIGNING_KEY),rpcConfigured:Boolean(RPC_URL),wssConfigured:Boolean(WSS_URL),persistence:DATA_DIR,received:state.received,verified:state.verified,envelopes:state.envelopes,rpcReconciled:state.rpcReconciled,wssSeen:state.wssSeen,duplicates:state.duplicates,manifestHash:SUBSCRIPTION_MANIFEST_HASH,reliability:{lastAuditAt:c.lastAuditAt,integrityErrors:c.integrityErrors,manifestDrift:c.manifestDrift,lastReconcileAt:c.lastReconcileAt},time:new Date().toISOString()});});
...
app.use(express.json({limit:"256kb"}));app.get("/",(_req,res:Response)=>res.json({ok:true,service:"shark-scout-alchemy-ab",version:VERSION,endpoints:["/health","/prospective/status","/reliability/status","/reliability/audit","/webhooks/alchemy/address-activity"]}));
...
app.listen(PORT,"0.0.0.0",()=>console.log(JSON.stringify({level:"info",event:"shark_scout_alchemy_ab_started",port:PORT,version:VERSION,signingKeyConfigured:Boolean(SIGNING_KEY),rpcConfigured:Boolean(RPC_URL),wssConfigured:Boolean(WSS_URL),dataDir:DATA_DIR,manifestHash:SUBSCRIPTION_MANIFEST_HASH,recoveredSourceEvents:recovered,packageC:true})));
