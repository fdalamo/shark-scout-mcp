...
const PROFILE_LIMIT = clamp(Number(process.env.HARVEST_PROFILE_LIMIT || 60), 0, 120);
const GLOBAL_WALLET_LIMIT = clamp(Number(process.env.HARVEST_GLOBAL_WALLET_LIMIT || 160), 40, 400);
...
async function heliusRecent(address:string){if(!HELIUS_API_KEY)return null;try{await heliusGate();const q=new URLSearchParams({"api-key":HELIUS_API_KEY,limit:"50"});const x=await fetchJson(`https://api.helius.xyz/v0/addresses/${address}/transactions?${q}`,{},3);return Array.isArray(x)?x:null;}catch{return null;}}
...
async function loadState():Promise<State>{try{const p=JSON.parse(await fs.readFile(STATE_PATH,"utf8"));return{schemaVersion:4,createdAt:p.createdAt||now(),updatedAt:p.updatedAt||now(),wallets:p.wallets||{},tokens:p.tokens||{},runs:Array.isArray(p.runs)?p.runs:[]};}catch{const t=now();return{schemaVersion:4,createdAt:t,updatedAt:t,wallets:{},tokens:{},runs:[]};}}
...
export async function runHarvest(){
  const startedAt=now(),state=await loadState(),before=Object.keys(state.wallets).length,providerErrors:string[]=[];let raw=0,dupes=0,screened=0,invalid=0,tagRejected=0,profiled=0,unknown=0;let tokens:string[]=[];
  ...
  for(const w of profileTargets){const h=summarizeHelius(await heliusRecent(w.address));w.lastProfiledAt=now();w.profile={...(w.profile||{}),heliusRecent:h};if(h){w.status="PROFILED";delete w.rejectionReason;profiled++;}else{w.status="UNKNOWN";w.rejectionReason="helius_profile_unavailable";unknown++;}}
  ...
  const report={schemaVersion:4,generatedAt:now(),telemetry,note:"Free-tier-safe adaptive harvest. Birdeye requests are paced/retried; entity validation focuses on new RAW wallets; Helius profiling rotates through unprofiled/stale wallets instead of repeatedly burning requests on the same leaders. Vybe remains reserved for free /v4/trades in the gauntlet.",rankedDiscoveryCandidates:top};await save(REPORT_PATH,report);return report;
}
